...
public class TileEntitySinteringFurnace extends TileEntity implements IInventory, ITickable {
	...
	public void setInventorySlotContents(int index, ItemStack other) {
		...
		boolean flag = !stack.isEmpty() && stack.isItemEqual(other) && ItemStack.areItemStackTagsEqual(stack, other);
		this.inventory.set(index, other);
		
		if(other.getCount() > this.getInventoryStackLimit())
			other.setCount(this.getInventoryStackLimit());
		if(index == 0 && index + 1 == 1 && !flag) {
			ItemStack stack1 = (ItemStack) this.inventory.get(index + 1);
			this.totalCookTime = this.getCookTime(other, stack1);
			this.cookTime = 0;
			this.markDirty();
		}
	}
	...
	public void update() {
 		boolean flag = this.isBurning(), flag1 = false;
	...
	public static int getItemBurnTime(ItemStack fuel) {
		if(fuel.isEmpty()) return 0;
			...
			if((item instanceof ItemTool || 
					...
					item instanceof ItemHoe) && ((ItemTool)item).getToolMaterialName().equals("WOOD")) return 200;
			
			if(item == Items.STICK) return 100;
			if(item == Items.COAL) return 2000;
			if(item == Items.LAVA_BUCKET) return 24000;
	...
	public boolean isUsableByPlayer(EntityPlayer player) {
		return world.getTileEntity(pos) != this ? false : player.getDistanceSq(
				(double)this.pos.getX() + 0.5,
				(double)this.pos.getY() + 0.5,
				(double)this.pos.getZ() + 0.5) <= 64;
	}
